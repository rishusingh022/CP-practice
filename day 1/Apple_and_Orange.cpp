...
//time limit excceded in 4 test case but same logic in python works for all test cases
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    vector<int>::iterator i;
    for(i=apples.begin();i!=apples.end();i++){
        *i=*i+a;
    }
    for(i=oranges.begin();i!=oranges.end();i++){
        *i=*i+b;
    }
    int count1=0;
    int count2=0;
...
int main()
{
    //s is the start point, and t is the endpoint.
